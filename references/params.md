# 参数说明

## 地址 JSON（--sender / --recipient）

```json
{
  "address": "北京市朝阳区融新科技中心A座",  // 完整地址
  "houseNumber": "3号楼1单元",               // 门牌号（地址簿返回或用户提供时填写，无则留空）
  "lat": 40019368,                            // 纬度，整数×1e6
  "lng": 116469191,                           // 经度，整数×1e6
  "name": "张三",                              // 联系人姓名（地址簿返回或用户提供时填写，无则留空）
  "phone": "138xxxx5678",                     // 联系电话（必须真实号码，地址簿返回的脱敏号码需通过 get_address_info 获取真实号码）
  "cityId": 110100                            // 城市ID，见城市表
}
```

> ⚠️ `houseNumber` 和 `name` 在地址簿返回或用户明确提供时应正常填写，会传递到下单接口的 `fetchHouseNumber`/`recipientHouseNumber` 和 `senderName`/`recipientName` 字段。

**单地址场景**（取号/帮搬/帮扔/医院帮忙/其他帮忙）：`--sender` 和 `--recipient` 传同一个地址 JSON。

---

## 物品 JSON（--goods）

```json
{
  "goodsName": "其他",
  "goodsWeight": 1,          // 物品实际重量（公斤数），值是多少就是多少公斤，如 15 表示 15kg。重量越大配送费越贵
  "goodTypes": [8],          // 物品类型数组，必须按下方映射表选择对应值，禁止直接复制此示例
  "goodTypeNames": ["其他"]  // 物品类型名称数组（与 goodTypes 一一对应）
}
```

> ⚠️ **严格查表规则**：`goodTypes` 数值与 `goodTypeNames` 名称必须**整行一起取**，禁止单独推断（如"餐饮"绝不是 `[1]`，"鲜花"绝不是 `[2]`）。每次都必须找到用户描述对应的那一整行，同时取出该行的 `goodTypes` 和 `goodTypeNames`，缺一不可。

| 用户描述关键词 | goodTypes（数值） | goodTypeNames（名称） | goodsWeight 默认值 |
|---|---|---|---|
| 餐饮、外卖、食物、饭、菜、奶茶、饮料 | **`[2]`** | **`["餐饮"]`** | 1 |
| 文件、合同、资料、单据、证件、文书 | **`[4]`** | **`["文件"]`** | 1 |
| 生鲜、水果、蔬菜、肉、海鲜 | **`[3]`** | **`["生鲜"]`** | 2 |
| 蛋糕、甜品 | **`[9]`** | **`["蛋糕"]`** | 2 |
| 鲜花、花束、花 | **`[1]`** | **`["鲜花"]`** | 1 |
| 数码、手机、电脑、平板、耳机、电子产品 | **`[5]`** | **`["数码"]`** | 1 |
| 服饰、衣服、鞋、包、服装 | **`[7]`** | **`["服饰"]`** | 1 |
| 快递、包裹、箱子 | **`[8]`** | **`["快递"]`** | 2 |
| 五金、工具、零件、螺丝、管件 | **`[2]`** | **`["五金"]`** | 2 |
| 汽配、汽车配件、车零件 | **`[2]`** | **`["汽配"]`** | 3 |
| 其他、物品、东西、不明确 | **`[8]`** | **`["其他"]`** | 1 |

> ⚠️ `goodsWeight` 值即公斤数。如用户明确说"15公斤"，传 `15`；说"5公斤左右"，传 `5`。默认值仅供参考，**以用户实际告知为准**。

---

## businessType 与 bizTypeSceneTag

`businessType` 只有两个合法值，**严禁传 3/4/5/6**（会导致 10002 错误）：

| 用户意图 | --business-type | --biz-type-scene-tag |
|---|---|---|
| 帮取送文件/物品（A→B） | 1 | 0（默认） |
| 餐厅取号 | 1 | 1 |
| 医院帮忙 | 1 | 2 |
| 其他取号 | 1 | 3 |
| 帮搬装 | 1 | 4 |
| 其他帮忙 | 1 | 5 |
| 帮扔杂物 | 1 | 6 |
| 帮买东西 | 2 | 0（默认即可，另需按购买方式传 `--business-type-tag`，见下） |

**purchaseGoodDetail**（CLI 参数名 `--purchase-detail`）：帮买场景（businessType=2）**必传**，如 `"一瓶矿泉水"`。其他场景留空。

---

## businessTypeTag

`businessTypeTag` 与 `bizTypeSceneTag` 是两个完全独立的参数，无关联。**仅在帮买场景（businessType=2）下生效**，用于区分购买地址来源：

> ⚠️ `--business-type-tag` **未列入 CLI 的 usage 帮助文本**（工具方遗漏），但参数功能正常，必须按本表正常传。以本文档为准，不要因 usage 未列出而省略。
>
> ⚠️ 两个参数名极易混淆：帮买传 `--business-type-tag`，帮取送体系（含取号/帮忙）传 `--biz-type-scene-tag`，**严禁互串**。误传不会报错而是**静默错单**（如帮买误传 `--biz-type-scene-tag 1`，会被服务器理解为「餐厅取号」）。

| 场景 | --business-type-tag | 含义 |
|------|---------------------|------|
| 帮买 - 用户指定购买地址 | `0` | 骑手前往 `--sender` 指定地址购买 |
| 帮买 - 就近购买 | `1` | 骑手自行决定购买地点，`--sender` 传收件地址 |
| 帮取送（businessType=1） | `0` | 不适用 |

---

## 坐标格式

整数×1e6（如 40.019368° → 40019368）。地址簿返回的 `lat`/`lng` 已是此格式，**可直接用于下单，无需转换**。

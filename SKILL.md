...
1. 执行 `login` 命令（内置一体化授权，无需外部依赖）：
   ```bash
   sh dist/run.sh login
   ```
2. `login` 命令自动完成全流程：检查本地缓存 → 获取授权链接 → 输出链接 → 轮询等待用户扫码 → 写入 Token。
3. 检查输出：
   - **输出 `✅ 已登录`** → 直接进入 Step 1
   - **输出 `AUTH_LINK: <url>`** → 需要授权，命令正在自动轮询等待
   - **任何接口返回 `code: 10000`**（Token 服务端过期） → 执行 `login --force` 强制重新授权
4. 当输出 `AUTH_LINK` 和 `AUTH_QRCODE` 时，按以下格式展示给用户：
...
> `{授权链接}`
>
> 也可使用美团 App 扫描下方二维码完成授权：
>
> ![美团授权二维码]({AUTH_QRCODE 输出的图片路径})

5. `login` 命令在后台持续轮询（最多 10 分钟），用户扫码完成后自动输出 `✅ 授权成功`，继续进入 Step 1。超时或失败则提示用户重新触发。
